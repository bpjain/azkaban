...
static struct net_generic *net_alloc_generic(void)
{
	...
	ng = kzalloc(generic_size, GFP_KERNEL|__GFP_NS);
#else
...
#ifdef CONFIG_AZKABAN_NET_NS
	net = kmem_cache_zalloc(KMEM_CACHE_NS(net_cachep), GFP_KERNEL);
#else
...
#ifdef CONFIG_AZKABAN_NET_NS
	kmem_cache_free(KMEM_CACHE_NS(net_cachep), net);
#else
			...
			struct user_namespace *user_ns, struct net *old_net)
{
	...
	if (flags & CLONE_NEWMEM) {
		...
		s = ns_copy_kmem_cache(net_cachep);
		if (IS_ERR(s))
